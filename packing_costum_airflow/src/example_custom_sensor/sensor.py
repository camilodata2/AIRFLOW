...
class AvailableDataSensor(BaseSensorOperator):
    def __init__(self, redshift_conn_id:str,start_date="{{ds}}",end_date="{{nex_ds}}",**kwargs):
        ...
        Args:
            redshift_conn_id (str): _description_
            start_date (str, optional): _description_. Defaults to "{{ds}}".
            end_date (str, optional): _description_. Defaults to "{{nex_ds}}".
            En Airflow, las variables como {{ ds }} y {{ next_ds }} 
            son utilizadas para referenciar fechas de ejecución
