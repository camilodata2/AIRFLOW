...
def s3_copy_operator_decorator(
    task_id: str,
...
def sagemaker_endpoint_operator_decorator(task_id, config):
    ...
    def sagemaker_endpoint_operator_task():
        return SageMakerEndpointOperator(
            task_id=task_id, wait_for_completion=True, config=config, dag=dag
        )
...
sagemaker_train_model_task = sagemaker_training_operator_decorator(
    ...
    config={
        ...
        "HyperParameters": {"max_depth": "63", "max_samples": "100"},
        "InputDataConfig": [
            {
                "ChannelName": "train",
                "DataSource": {
...
processing_data = PythonOperator(
    task_id="processing_data", python_callable=extract_titanic_data, dag=dag
)

# Task dependencies
(
    Label("download_date_task")
    >> Label("processing_data")
    >> Label("sagemaker_train_model_task")
    >> Label("sagemaker_deploying_model_task")
)
